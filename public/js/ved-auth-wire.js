...
(() => {
  ...
  const API_BASE = (() => {
    try{
      const m = document.querySelector('meta[name="ved-api-base"]')?.content?.trim()?.replace(/\/+$/,'');
      if (m) return m;
      if (location.origin && !location.origin.startsWith('file://')) return location.origin.replace(/\/+$/,'') + '/api';
    }catch{}
  ...
  if (regForm){
    ...
    if (sendOtpBtn){
      sendOtpBtn.addEventListener('click', async ()=>{
        ...
        try{
          const r = await fetch(api('/api/send-otp'), {method:'POST',credentials:'include',
    ...
    regForm.addEventListener('submit', async (e)=>{
      ...
      try{
        const r = await fetch(api('/api/register'), {method:'POST',credentials:'include',
        ...
        if(r.ok){
          ...
          if(otp){
            const vr = await fetch(api('/api/verify-otp'), {method:'POST',credentials:'include',
  ...
  if (verifyForm){
    verifyForm.addEventListener('submit', async (e)=>{
      ...
      try{
        const r = await fetch(api('/api/verify-otp'), {method:'POST',credentials:'include',
          headers:{'Content-Type':'application/json'}, body: JSON.stringify({identifier:ident, otp})});
        const d = await r.json();
  ...
  if (pfSave){
    ...
    pfSave.addEventListener('click', async ()=>{
      ...
      try{ const r=await authFetch('/api/user/profile',{method:'POST',body:JSON.stringify({name,handle})});
           banner(r.ok?'Saved':'Saved locally'); }catch{ banner('Saved locally'); }
    });
